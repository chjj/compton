...
*--backend* 'BACKEND'::
	Specify the backend to use: `xrender`, `glx` or `xr_glx_hybrid`. GLX (OpenGL) backend generally has much superior performance as far as you have a graphic card/chip and driver. Issues with GLX like stuttering (especially in combination with the use of xft fonts) might be resolved using the hybrid backend.
